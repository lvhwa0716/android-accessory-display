...
public class UsbAccessoryStreamTransport extends Transport {
    ...
    public UsbAccessoryStreamTransport(Logger logger, ParcelFileDescriptor fd,EventNotify ev) {
        super(logger, 16384,ev);
        mFd = fd;
    ...
    protected void ioClose() {
    	getLogger().log("UsbAccessoryStreamTransport ioClosed");
        try {
            mFd.close();
    ...
    protected int ioRead(byte[] buffer, int offset, int count) throws IOException {
        if (mInputStream == null) {
        	mEventNotify.eventIOError(1/* read error */);
    ...
    protected void ioWrite(byte[] buffer, int offset, int count) throws IOException {
        if (mOutputStream == null) {
        	mEventNotify.eventIOError(2/* write error */);
            throw new IOException("Stream was closed.");
        }
