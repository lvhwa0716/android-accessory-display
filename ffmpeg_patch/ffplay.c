...
#include "config.h"
#undef CONFIG_AVFILTER // no need it , lvhwa0716
#include <inttypes.h>
#include <math.h>
#include <limits.h>
#include <signal.h>
...
{
	av_log(NULL, AV_LOG_WARNING, "do_exit");
#if INJECT_EVENT2PIPE
	if(is->inject_event_fd > 0 ){
		close(is->inject_event_fd);
	}
#endif
    if (is) {
        stream_close(is);
...
static void event_loop(VideoState *cur_stream)
{
    ...
    for (;;) {
    ...
    #if INJECT_EVENT2PIPE
        case SDL_KEYUP:
			if(cur_stream->inject_event_fd > 0) {
				struct inject_event_struct ie;
				memset(&ie,0, sizeof(ie));
				ie.type = 1;
				ie.status = 0;
				ie.code = event.key.keysym.sym;
				write(cur_stream->inject_event_fd,&ie,sizeof(ie));
			}
            break;
        case SDL_KEYDOWN:
			if(cur_stream->inject_event_fd > 0) {
		...
		case SDL_MOUSEBUTTONDOWN: {
				...
				if(down != 0) {
					if(cur_stream->inject_event_fd > 0) {
						...
						ie.x = event.button.x;
						ie.y = event.button.y;
						ie.screen_w = cur_stream->width;
						ie.screen_h = cur_stream->height;
						ie.video_x = rect.x;
						ie.video_y = rect.y;
						ie.video_w = rect.w;
						ie.video_h = rect.h;
        ...
        case SDL_MOUSEMOTION: {
				...
				if (event.motion.state & 0x01) {
					down = 0x1;
				}
				if (event.motion.state & 0x02) {
					down = down | 0x02;
				}
