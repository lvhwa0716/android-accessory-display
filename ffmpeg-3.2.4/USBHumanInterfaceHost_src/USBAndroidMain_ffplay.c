...
static const char *window_title="Android Screen Mirror";
...
#if 1 // lvhwa
    {
    	// for speed up probe media stream , lvhwa

    	//perhaps raise : Codec not find

		int size = DEFAULT_WIDTH * DEFAULT_HEIGHT;
		if(size > 1280 * 720) {
			ic->probesize = 50*1024;
			ic->max_analyze_duration = 3000;
		} else if(size > 640 * 480){
			ic->probesize = 20*1024;
			ic->max_analyze_duration = 2000;
		}else {
			ic->probesize = 10*1024;
        ...
        case SDL_MOUSEMOTION: {
			...
			if (event.motion.state & SDL_BUTTON_MMASK) {
				...
				struct _hid_data _hid;
					...
					_hid.type = 2;
					_hid.status = down;
					_hid.x = event.motion.x;
					_hid.y = event.motion.y;
					_hid.dx = event.motion.xrel;
					_hid.dy = event.motion.yrel;
					_hid.video_x = rect.x;
					_hid.video_y = rect.y;
					_hid.video_w = rect.w;
					_hid.video_h = rect.h;
...
#endif
{
	...
	//av_log_set_level(AV_LOG_TRACE); // lvhwa only for debug
